...
from tinygrad.runtime.ops_cuda import SASSCompiler, CUDACompiler
...
def info(bufs_cuda, bufs_sass):
  ret = {}
  cuda_vals, sass_vals = [np.frombuffer(buf.as_buffer(), dtype=buf.dtype.np) for buf in [bufs_cuda, bufs_sass]]
  mask = ~(np.isnan(cuda_vals) & np.isnan(sass_vals))
  ret["cuda"], ret["sass"] = cuda_vals.tolist(), sass_vals.tolist()
  if cuda_vals[mask].shape == sass_vals[mask].shape and len(cuda_vals[mask]) > 0:
    ret["sum_diff"] = np.abs(np.sum(cuda_vals[mask] - sass_vals[mask])).item()
    ret["max_diff"] = np.abs(np.max(cuda_vals[mask] - sass_vals[mask])).item()
  return ret
  ...
  for num,ast in list(enumerate(ast_strs))[getenv("START", 0):getenv("END", len(ast_strs))]:
    ...
    try:
      print(f"{num:>5}/{len(ast_strs)} ({(num + 1) * 100.0 / len(ast_strs):.2f}%)\t"
            f"cuda: {cuda_prg(cuda_bufs, {}, wait=True)*1e6:7.2f} us\tsass: {sass_prg(sass_bufs, {}, wait=True)*1e6:7.2f} us")
    except Exception as e:
      print(e)
